...
@Service
class BucketService(private val s3Client: S3Client, private val s3Presigner: S3Presigner) {
  @Value(value = "\${bucket.name}") lateinit var bucketName: String
  ...
  fun upload(file: MultipartFile): String {
    try {
      ...
      s3Client.putObject(fileUploadRequest, RequestBody.fromBytes(file.bytes))

      this.s3Client
        .waiter()
        .waitUntilObjectExists(
          { it.bucket(bucketName).key(newKey) },
          { it.waitTimeout(Duration.ofSeconds(waiterDuration)) },
        )

      return newKey
    } catch (e: S3Exception) {
      throw IllegalArgumentException("File Deletion Failed: ${e.message}", e)
    }
  }
    ...
    try {
      ...
      this.s3Client
        ...
        .waitUntilObjectNotExists(
          { it.bucket(bucketName).key(fileKey) },
          { it.waitTimeout(Duration.ofSeconds(waiterDuration)) },
        )
  ...
  fun update(file: MultipartFile, fileKey: String): String {
    this.delete(fileKey)
  ...
  fun getSignedUrl(fileKey: String): String {
    try {
      val fileUpload = GetObjectRequest.builder().bucket(bucketName).key(fileKey).build()
