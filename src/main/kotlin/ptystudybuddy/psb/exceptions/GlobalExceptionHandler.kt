...
class GlobalExceptionHandler {
  ...
  ): ResponseEntity<ExceptionRes<List<Map<String, String?>>>> {
    val errors =
      e.bindingResult.fieldErrors.map { mapOf("field" to it.field, "message" to it.defaultMessage) }
    val body =
      mapOf("success" to false, "statusCode" to HttpStatus.BAD_REQUEST.value(), "content" to errors)
    return ResponseEntity.badRequest()
      .body(ExceptionRes(statusCode = e.statusCode.value(), errors = errors))
  }
  ...
  fun handleSpecificException(ex: Exception): ResponseEntity<ExceptionRes<String>> {
    val status =
      when (ex) {
    ...
    return ResponseEntity.status(status.value())
      .body(
        ExceptionRes(
          ...
          errors = ex.message ?: "Un error del tipo ${status.name} ocurrió",
        )
      )
  }
  ...
  fun handleUnsupportedException(
    ex: HttpMediaTypeNotSupportedException
  ): ResponseEntity<ExceptionRes<String>> {
