...
@Service
class SendGridService(private val sendGrid: SendGrid) {
  ...
  private fun emailBuilder(template: TemplateId, personalization: Personalization): Mail {
    return Mail().apply {
      from = sender
  ...
  private fun personalizationBuilder(
    ...
    return Personalization().apply {
      recipients.forEach { addTo(it) }
      data.forEach { (key, value) -> addDynamicTemplateData(key, value) }
    }
  }

  fun sendEmailAfterDecision(fullName: String, tutorEmail: String, status: Boolean) {
    if (status) {
      ...
      val mapForDisapproval =
        mapOf(
          "tutor_name" to fullName,
          "today_date" to DateAndTimeHelper.formatDate(LocalDate.now()),
        )
      sendEmailRequest(
        emailBuilder(
  ...
  fun sendEmailAfterSessionCancel(
    ...
    sendEmailRequest(
      emailBuilder(TemplateId.SESSION_CANCELLATION, personalizationBuilder(studentEmails, map))
    )
  }
}
